...
This repository is the staged foundation for the final product. The current implementation includes a runnable FastAPI backend, deterministic local store, synchronous worker pipeline, synthetic checkout incident fixtures, tests, a Next.js workbench shell, and deployment scaffolding.

## Architecture

- `apps/api`: FastAPI API, Pydantic v2 schemas, auth/session handling, mocked GitHub Copilot device-code auth for local tests, SQLAlchemy model shape, and local in-memory analysis store.
...
Remaining staged work is tracked in `docs/operations.md`. The main gaps are production persistence adapters, real object storage upload flows, real Copilot token exchange/runtime calls, Temporal activity idempotency backed by durable state, ClickHouse/OpenSearch indexing, RBAC policy expansion, Playwright e2e coverage, and production observability wiring.
