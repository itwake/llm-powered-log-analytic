...
def test_ai_platform_httpx_verify_uses_configured_ca_bundle() -> None:
    assert (
        Settings(ai_platform_ca_bundle="/etc/ssl/corp-root-ca.pem").ai_platform_httpx_verify()
        == "/etc/ssl/corp-root-ca.pem"
    )


def test_ai_platform_httpx_verify_can_be_disabled_outside_production() -> None:
    assert Settings(ai_platform_tls_verify=False).ai_platform_httpx_verify() is False


def test_ai_platform_httpx_client_kwargs_enable_env_proxy_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_proxy_env(monkeypatch)

    kwargs = Settings().ai_platform_httpx_client_kwargs()

    assert kwargs["trust_env"] is True
    assert "proxy" not in kwargs


def test_ai_platform_httpx_client_kwargs_read_https_proxy_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_proxy_env(monkeypatch)
    monkeypatch.setenv("HTTPS_PROXY", "http://windows-proxy.example:8080")

    kwargs = Settings(ai_platform_proxy_url=None).ai_platform_httpx_client_kwargs()

    assert kwargs["proxy"] == "http://windows-proxy.example:8080"
    assert kwargs["trust_env"] is True


def test_ai_platform_httpx_client_kwargs_read_lowercase_proxy_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_proxy_env(monkeypatch)
...
def test_ai_platform_httpx_client_kwargs_include_explicit_proxy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_proxy_env(monkeypatch)
    ...
    kwargs = Settings(
        ai_platform_proxy_url="http://proxy.example:8080",
        ai_platform_trust_env=False,
...
def test_ai_platform_httpx_client_kwargs_can_ignore_env_proxy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_proxy_env(monkeypatch)
    monkeypatch.setenv("HTTPS_PROXY", "http://env-proxy.example:8080")
...
def test_production_settings_reject_disabled_ai_platform_tls_verification() -> None:
    app_settings = Settings(
        env="production",
        secret_key="s" * 32,
        credential_encryption_key="c" * 32,
        ai_platform_tls_verify=False,
    )
