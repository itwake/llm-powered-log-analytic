...
async def test_pipeline_checkout_incident_end_to_end() -> None:
    ...
    assert all(
        "extension_seam" not in methods.get("granger_linear", {}) for methods in method_evidence
    )
    ...
    assert all(
        "pgem" in edge.method for edge in edges if edge.evidence["methods"]["pgem"]["supported"]
    )
    assert all(
