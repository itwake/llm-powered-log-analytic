...
│   ├── store.py              # MetadataStore Protocol, record dataclasses, sanitize_* redaction, RBAC tables, InMemoryStore, create_store
...
│   ├── observability.py      # configure_logging/otel, install_metrics, PIPELINE_STEP_NAMES, label sanitizers
...
│   │   ├── cases.py          # ~1600-line core router: cases, uploads, analysis runs, the five views
...
**Shared constant.** `observability.PIPELINE_STEP_NAMES` must match the workers' pipeline step list.
...
| Throwaway in-memory store | `LOGAN_STORE_BACKEND=memory` |
...
python -m pytest tests/api/test_api.py -q                 # API behavior on InMemoryStore
python -m pytest tests/api/test_sqlalchemy_persistence.py -q  # store-parity / persistence on SQLAlchemy
...
- **MetadataStore parity** — `InMemoryStore` (`store.py`) and `SQLAlchemyStore` (`sqlalchemy_store.py`) must behave identically.
...
4. `apps/api/app/store.py` — `MetadataStore` Protocol, record types, redaction helpers, `run_analysis` orchestration, `create_store`.
5. `apps/api/app/api/cases.py` — the core request lifecycle: cases, uploads, analysis runs, the five views.
...
9. `apps/api/app/sqlalchemy_store.py` — skim `__init__`/`_session`, `run_analysis`, `_fan_out_analysis_result`, `get_report_*` for the parity + SQL-view mechanism.
10. `apps/api/app/models/tables.py` + `apps/api/app/observability.py` — relational schema + metrics/`PIPELINE_STEP_NAMES`.
...
- **Add a `MetadataStore` capability** — add the signature to the `MetadataStore` Protocol in `store.py`, implement it on `InMemoryStore`, implement identical behavior on `SQLAlchemyStore` (add ORM columns in `models/tables.py` + a Postgres migration in `migrations/` if schema changes), and cover it in **both** `tests/api/test_api.py` and `tests/api/test_sqlalchemy_persistence.py`.
- **Add a view / read endpoint served by SQL** — add the route in `cases.py` using the `_query_report`/`getattr` fallback pattern, implement `get_report_<name>` on `SQLAlchemyStore` over the normalized tables (and optionally external analytics), and ensure the InMemory recompute branch returns the same JSON shape.
...
- **Wire fakes in a test** — `create_app(store=InMemoryStore(Settings(...)), model_gateway=MockAIPlatformAnnotationGateway(), s3_client_factory=lambda _: FakeS3Client())`; drive it with httpx `AsyncClient` over `ASGITransport`; authenticate via `store.register_user` + `store.create_session` and set the `logan_session` cookie (see `_authenticated_client` in `tests/api/test_api.py`).
...
- **Two-store parity is mandatory.** Every store change lands in **both** `store.py` (`InMemoryStore`) and `sqlalchemy_store.py` (`SQLAlchemyStore`); `SQLAlchemyStore` reuses helpers imported from `store.py`, including private ones like `_case_role_allows` and `_slugify`. Verified by the two test suites above.
- **Process env only.** `Settings` is a frozen dataclass evaluated at import time — the app never loads `.env`. Tests construct a fresh `Settings(...)` and pass it into `create_store`/`InMemoryStore` rather than mutating env.
...
- **View fallback pattern.** `cases.py` `_query_report` does `getattr(store, "get_report_*")`: `InMemoryStore` lacks these methods (returns `None` → the route recomputes from the in-memory `AnalysisResult`), while `SQLAlchemyStore` answers from normalized SQL (or external analytics when `LOGAN_EXTERNAL_ANALYTICS_QUERIES_ENABLED`). Keep both paths returning the same shape.
...
- **`PIPELINE_STEP_NAMES` sync.** Adding a pipeline step requires updating `observability.PIPELINE_STEP_NAMES` **and** `scripts/full_stack_smoke.py` `PIPELINE_STEPS`, then re-running the benchmark.
