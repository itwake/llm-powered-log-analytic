    ...
    *,
    ...
    for template in templates:
        ...
        if not unique_by_log:
            fallback = group[
                int(hashlib.sha256(template.template_id.encode()).hexdigest(), 16) % len(group)
            ]
            unique_by_log[fallback.log_id] = ("random_fallback", fallback)

        for rank, (reason, line) in enumerate(
            list(unique_by_log.values())[:max_samples_per_template]
        ):
