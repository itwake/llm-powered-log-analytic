...
def _fallback_summary(
    *,
    ...
    next_action_lines = [f"- {action['title']}: {action['description']}" for action in next_actions]
    markdown = "\n".join(
        [
    ...
    confidence = round(sum(edge_confidence) / len(edge_confidence), 4) if edge_confidence else 0.0
