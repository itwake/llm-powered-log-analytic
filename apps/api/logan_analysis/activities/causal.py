...
from logan_analysis.algorithms.causal_granger import score_granger_pairs
from logan_analysis.algorithms.causal_pgem import score_pgem_transition
from logan_analysis.algorithms.causal_series import (
    build_count_series,
    derive_granger_max_lag_bins,
)
from logan_analysis.algorithms.pagerank import pagerank
...
def _service_relation(
    source_lines: list[NormalizedLogLine], target_lines: list[NormalizedLogLine]
) -> bool:
...
def _support(
    source_times: list[datetime], target_times: list[datetime], max_lag_seconds: int
) -> tuple[int, int | None]:
    lags: list[int] = []
...
def infer_causal_graph(
    *,
    ...
    offending_template_ids = {
        line.template_id
        for line in logs
        if line.template_id and line.golden_signal in OFFENDING_SIGNALS
    }
    ...
    for template_id in offending_template_ids:
        ...
        first = template.first_seen or min(
            (line.timestamp for line in group if line.timestamp), default=None
        )
        last = template.last_seen or max(
            (line.timestamp for line in group if line.timestamp), default=None
        )
    ...
    for node in nodes:
        ...
        severity = max(
            (line.severity_score for line in lines_by_template[node.template_id]), default=0
        )
