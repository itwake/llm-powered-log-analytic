...
from logan_analysis.evaluation.schemas import (
...
def _evaluate_root_cause(
    ...
    for candidate in result.causal_graph.root_cause_candidates[:top_k]:
        ...
        matched = [pattern for pattern in patterns if _matches(pattern, template.template_text)]
...
async def evaluate_benchmark(benchmark: LoadedBenchmark) -> BenchmarkEvaluationReport:
    ...
    threshold_results = [score.passed for score in metrics.values() if score.passed is not None]
