...
file below `LOGAN_LOCAL_OBJECT_STORE_DIR`. Completion verifies size and SHA-256 and is idempotent
for the same digest. Analysis accepts completed `input_file_ids`; test and local tooling may also
pass filesystem `input_paths`.
